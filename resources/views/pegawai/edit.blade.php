<x-layout title="Edit Pegawai✨">
    <div class="p-10">
        ...
        <div class="bg-gray-200 px-3 py-5 max-w-md mx-auto rounded-lg">
            <form action="{{route('pegawai.update', $data->id)}}" method="post" class="max-w-sm mx-auto">
                @method('put')
                @csrf
                <div class="mb-2">
                    <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama</label>
                    <input type="text" name="nama" id="nama" class="shadow-sm bg-gray-50 border border-gray-300
            text-gray-900
            text-sm
            ...
            rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700
            dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500
            dark:focus:border-blue-500 dark:shadow-sm-light" value="{{$data->nip}}"/>
                    @error('name')
                    <div class="text-sm text-red-500">{{ $message }}</div>
                    @enderror
                </div>
                <div class="mb-5">
                    ...
                    >
                        <option value="{{$data->is_active}}" selected hidden="">{{$data->is_active}}</option>
                        <option>aktif</option>
            ...
            rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700
            dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500
            dark:focus:border-blue-500 dark:shadow-sm-light" value="{{$data->jabatan}}"/>
                    @error('jabatan')
                    <div class="text-sm text-red-500">{{ $message }}</div>
                    @enderror
                </div>
