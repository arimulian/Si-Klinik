<x-layout title="Edit Daftar Dokter">
    <div class="p-10">
        <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Edit Daftar Dokter</h1>
        <div class="bg-gray-200 px-3 py-5 max-w-md mx-auto rounded-lg">
            <form action="{{ route('dokter.update', $data->id) }}" method="post" class="max-w-sm mx-auto">
                @method('put')
                @csrf
                <div class="mb-2">
            ...
            dark:focus:border-blue-500 dark:shadow-sm-light" value="{{$data->user->name}}"/>
                    @error('name')
            ...
            rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700
            dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500
            dark:focus:border-blue-500 dark:shadow-sm-light" value="{{$data->user->email}}"/>
                    @error('email')
                    <div class="text-sm text-red-500">{{ $message }}</div>
                    @enderror
                </div>
            ...
            dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500
            dark:focus:border-blue-500 dark:shadow-sm-light" value="{{$data->spesialis}}"/>
                    @error('specialis')
                    <div class="text-sm text-red-500">{{ $message }}</div>
                    @enderror
                </div>
