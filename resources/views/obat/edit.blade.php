<x-layout title="Edit Daftar Obat">
    <div class="p-10">
        <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Edit Daftar Obat</h1>
        <div class="bg-gray-200 px-3 py-5 max-w-md mx-auto rounded-lg">
            <form action="{{ route('obat.update', $data->id)}}" method="post" class="max-w-sm mx-auto">
                @method('put')
                @csrf
                <div class="mb-2">
            ...
            dark:focus:border-blue-500 dark:shadow-sm-light" value="{{$data->nama_obat}}"/>
                    @error('name')
